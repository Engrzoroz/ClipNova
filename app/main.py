...
app = FastAPI(title="ClipNova", version="3.1.0")
...
def ytdlp_runtime_cli_args() -> list[str]:
    return [
        "--js-runtimes", "node",
...
def ytdlp_metadata_options() -> dict:
    """
    Metadata-only extraction should not force the mweb playback client or
    select a downloadable media format. This keeps Analyze working even
    when YouTube temporarily withholds playable formats from a cloud IP.
    """
    opts = {
        "quiet": True,
        "no_warnings": True,
...
def pot_provider_available() -> bool:
    try:
        with socket.create_connection(("127.0.0.1", 4416), timeout=0.35):
            return True
    except OSError:
        return False
...
def classify_processing_error(raw: str) -> tuple[str, str]:
    ...
    if "n challenge solving failed" in lower or "no supported javascript runtime" in lower:
        return (
            "YOUTUBE_JS_CHALLENGE",
            "YouTube's JavaScript challenge could not be solved by the server. ClipNova V3 requires its Node.js/EJS runtime to be healthy.",
        )

    if "po token" in lower or "only images are available" in lower:
        return (
            "YOUTUBE_PO_TOKEN",
            "YouTube did not provide usable media formats to this server. The PO-token provider may be unavailable or YouTube may have changed its playback checks.",
        )

    if "requested format is not available" in lower:
        return (
            "FORMAT_UNAVAILABLE",
            "YouTube did not expose a usable stream for this request. This can be caused by YouTube playback checks, not only by the quality setting. Try again after the server finishes starting, then try 720p or Best available.",
        )
...
@app.get("/api/health")
def health():
    return {"ok": True, "service": "ClipNova", "version": "3.1.0"}
...
def server_status():
    ...
    return {
        ...
        "pot_provider": pot_provider_available(),
    }
...
def analyze(req: AnalyzeRequest):
    ...
    try:
        with yt_dlp.YoutubeDL(ytdlp_metadata_options()) as ydl:
            # process=False avoids yt-dlp's default format selection during
            # metadata analysis. The previous V3 code could fail here with:
            # "Requested format is not available" before rendering even began.
